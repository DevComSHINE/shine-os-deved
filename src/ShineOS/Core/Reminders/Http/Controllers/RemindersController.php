...
class RemindersController extends Controller {
    ...
    public function createReminder($patient_id) {
        ...
        if($data['Patients']->patientContact['mobile'] != NULL || $data['Patients']->patientContact['email'] != NULL) {
            ...
            return Redirect::to('records#visit_list')
    ...
    public function insertBroadcast() {
        ...
        $smsmessage = "You received a broadcast message from ShineOS+: ".$subject.". Check your ShineOS account now.";
    ...
    */
    ...
    function cronReminders()
    {
        ...
        $reminders = ReminderMessage:: whereRaw('DATE_FORMAT(`reminder_message`.`appointment_datetime` - INTERVAL `reminder_message`.`daysbeforesending` DAY, "%Y-%m-%d") = "'.$today.'"')
                    ->where('reminder_message.sent_status', 'SET')
        ...
        foreach($reminders as $reminder) {
            ...
            if($patient->patientContact->email) {
                $sendEmail = $this->sendToEmail($patientName, $patient->patientContact->email, $reminder->reminder_subject, $reminder->reminder_message, $reminder->appointment_datetime);
            }
            ...
            if($patient->patientContact->mobile) {
                $mob = '63'.substr( str_replace("-", "", $patient->patientContact->mobile), 1);
                 $ChikkaSMS = new ChikkaSMS;
                 $sendText = $ChikkaSMS->sendText($reminder->remindermessage_id, $mob, $reminder->reminder_message);
            }
    ...
    public function testemail()
    {
        $data = array(
                ...
                'fromFacility' => "ShineOS Test Clinic"
                );
